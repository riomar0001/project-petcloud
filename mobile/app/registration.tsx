...
import { View, Text, ScrollView, KeyboardAvoidingView, Platform, TouchableOpacity, Animated, Dimensions } from 'react-native';
...
export default function RegisterScreen() {
  ...
  const updateField = (field: keyof RegisterFormData, value: string | boolean): void => {
  ...
  const getPasswordStrength = (): {
    ...
    if (score === 3) return { level: 3, label: 'Good', color: 'bg-mountain-meadow-400' };
  ...
  const renderStep1 = () => (
    <View>
      <View className="mb-5">
        ...
        <Text className="mt-1 text-sm text-gray-500">Tell us about yourself</Text>
      </View>
  ...
  const renderStep2 = () => (
    <View>
      <View className="mb-5">
        <Text className="text-xl font-bold text-gray-900">Create Password</Text>
        <Text className="mt-1 text-sm text-gray-500">Secure your account with a strong password</Text>
      </View>
      ...
      {formData.password.length > 0 && (
        <View className="-mt-2 mb-4">
          <View className="mb-1.5 flex-row gap-1.5">
            {[1, 2, 3, 4].map((i) => (
              <View key={i} className={`h-1 flex-1 rounded-full ${i <= passwordStrength.level ? passwordStrength.color : 'bg-gray-200'}`} />
            ))}
          </View>
          <Text className="text-xs text-gray-500">
            Password strength: <Text className="font-semibold">{passwordStrength.label}</Text>
          </Text>
        </View>
      )}
      ...
      <View className="rounded-xl border border-gray-100 bg-gray-50 p-4">
        <Text className="mb-2 text-xs font-semibold text-gray-500">PASSWORD REQUIREMENTS</Text>
        {[
          ...
          <View key={i} className="mt-1.5 flex-row items-center">
            <Ionicons name={req.met ? 'checkmark-circle' : 'ellipse-outline'} size={16} color={req.met ? '#059666' : '#D1D5DB'} />
            <Text className={`ml-2 text-xs ${req.met ? 'text-mountain-meadow-700' : 'text-gray-400'}`}>{req.text}</Text>
          </View>
        ))}
      </View>
    </View>
  ...
  const renderStep3 = () => (
    <View>
      <View className="mb-5">
        <Text className="text-xl font-bold text-gray-900">Review & Confirm</Text>
        <Text className="mt-1 text-sm text-gray-500">Almost done! Verify your details</Text>
      </View>
      ...
      <View className="mb-5 rounded-xl border border-gray-100 bg-gray-50 p-4">
        {[
          ...
          <View key={i} className={`flex-row items-center py-3 ${i > 0 ? 'border-t border-gray-200' : ''}`}>
            ...
            <View className="flex-1">
              ...
              <Text className="text-sm font-medium text-gray-900">{item.value}</Text>
            </View>
            <TouchableOpacity
              onPress={() => {
                animateTransition('back');
      ...
      >
        <View
          className={`mr-3 mt-0.5 h-5 w-5 items-center justify-center rounded-md border-2 ${
            formData.acceptTerms ? 'border-mountain-meadow-600 bg-mountain-meadow-600' : errors.acceptTerms ? 'border-red-400' : 'border-gray-300'
          }`}
        >
        ...
        <View className="flex-1">
          <Text className="text-sm leading-5 text-gray-700">
            I agree to the <Text className="font-semibold text-mountain-meadow-600">Terms of Service</Text> and{' '}
            <Text className="font-semibold text-mountain-meadow-600">Privacy Policy</Text>
          </Text>
          {errors.acceptTerms && <Text className="mt-1 text-xs text-red-500">{errors.acceptTerms}</Text>}
        </View>
      </TouchableOpacity>
    </View>
  );
    ...
    <SafeAreaView className="flex-1 bg-white">
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        ...
        <View className="flex-row items-center justify-between px-5 pb-2 pt-3">
          <TouchableOpacity onPress={handleBack} className="h-10 w-10 items-center justify-center rounded-full bg-gray-100" activeOpacity={0.7}>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <View className="flex-1 px-6 pb-8 pt-2">
            <ProgressSteps currentStep={currentStep} totalSteps={TOTAL_STEPS} labels={STEP_LABELS} />
        ...
        <View className="border-t border-gray-100 px-6 pb-4 pt-3">
          {currentStep < TOTAL_STEPS ? (
            <AppButton title="Continue" onPress={handleNext} variant="primary" icon={<Ionicons name="arrow-forward" size={18} color="#FFFFFF" />} />
          ) : (
            <AppButton
              ...
              loading={loading}
              variant="primary"
              icon={!loading ? <Ionicons name="checkmark-circle" size={18} color="#FFFFFF" /> : undefined}
            />
          )}
          ...
          <View className="mt-3 flex-row items-center justify-center">
            ...
            <TouchableOpacity onPress={() => router.push('/')}>
              <Text className="text-sm font-semibold text-mountain-meadow-600">Sign In</Text>
            </TouchableOpacity>
          </View>
        </View>
